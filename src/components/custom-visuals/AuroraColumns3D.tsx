...
const AuroraColumns3D: React.FC<VisualProps> = ({ width, height }) => {
  ...
  useEffect(() => {
    ...
    const geo = new THREE.CylinderGeometry(1.3, 1.3, 10, 8, 1, true);
    const mat = new THREE.MeshPhongMaterial({ color: 0x333333, transparent: true, opacity: 0.6 });
    const inst = new THREE.InstancedMesh(geo, mat, total); scene.add(inst);
    const dummy = new THREE.Object3D();

    const center = new THREE.Vector2(cols/2, rows/2);
    let i=0; const phases: number[] = [];
    for (let y=0;y<rows;y++) for (let x=0;x<cols;x++){
      const px = (x - cols/2)*spacing; const pz = (y - rows/2)*spacing;
      dummy.position.set(px, -30, pz); dummy.updateMatrix(); inst.setMatrixAt(i, dummy.matrix);
    ...
    const animate = () => {
      ...
      for (let j=0;j<total;j++){
        ...
        dummy.scale.set(1, h/10, 1); inst.setMatrixAt(j, dummy.matrix);
      }
